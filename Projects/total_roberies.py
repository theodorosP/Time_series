...
'''
series2 = pd.concat([test.shift(1), test], axis = 1)
series2.columns = ["predicted", "expected"]
print(series2)
expected = list()
predicted = list()
for i in range(1, len(series2)):
    expected.append(series2["expected"][i])
    predicted.append(series2["predicted"][i])
...
#test harness, vallidation dataset 10% fo the dataset
...
#save ARIMA model
transformed, lam = scipystats.boxcox(series1.squeeze())
model = stm.tsa.arima.ARIMA (transformed, order = (1, 1, 1))
model_fit = model.fit()
...
rmse = sqrt(mean_squared_error(predictions, series_validation.values))
print(rmse)
